...
export const Card = ({
  ...
  <div
    className={`rounded-xl w-full shadow overflow-hidden ${
      webProperty === "ousd" ? "text-white border-white border-2" : "text-black"
    } bg-white`}
  >
    <div className="w-full relative">{img}</div>
    <div className="p-6 w-full relative">
      <div className="space-y-3.5">
        <div className={`${webProperty === "story" ? "w-3/4" : ""}`}>
          <Typography.Title
            className={`${webProperty === "launchpad" ? "mt-3" : "mt-2"}`}
          >
            {title}
          </Typography.Title>
        </div>
        <Typography.Body2>{body}</Typography.Body2>
        <div>
          <a
            href={linkHref}
            className="font-sansInter font-bold text-base text-link-blue mr-1.5"
          >
            {linkText}
          ...
          {linkText2 && (
            <Typography.Link
              href={linkHref2}
              className="mt-1 space-x-4 flex items-center"
            >
              {linkText2}
            </Typography.Link>
          )}
        ...
        <div
          className={`absolute top-0 mt-0 transform overflow-hidden ${
            webProperty === "story"
              ...
              : "left-6 -translate-y-2/3 rounded-xl hidden lg:block"
          }`}
        >
          <div
            className={`${
              webProperty === "story" ? "h-24 w-24" : "h-20 w-20"
            } relative`}
          >
            {thumbnail}
          </div>
